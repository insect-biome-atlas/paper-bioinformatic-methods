...
i <- snakemake@wildcards$eee_run
ord <- snakemake@wildcards$order

if (file.size(snakemake@input$discarded) > 0) {
     ee_set <- read.delim(snakemake@input$discarded, header=FALSE)
     ee_set <- ee_set$V1[ee_set$V1 %in% taxonomy$ASV]
     counts_subset <- counts[!(rownames(counts) %in% ee_set),]
     evodistlist_subset <- evodistlist[!(evodistlist$asv1 %in% ee_set) & !(evodistlist$asv2 %in% ee_set),]
} else {
     ee_set <- character(0)
